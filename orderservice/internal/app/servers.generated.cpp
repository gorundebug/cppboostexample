...
void ServiceServers::initInfrastructure(
  ...
  launch([](ServiceGenerated* service,
            ...
            std::exception_ptr* first_maker_error)
             -> boost::asio::awaitable<void> {
    try {
      service->clients_.inventory_service_api_client = co_await service->makers_.inventory_service_api_client(
          maker_context, *service, connector_config);
      if (!service->clients_.inventory_service_api_client) {
  ...
  }(&service, cfg.dataConnectors.inventoryServiceApi, maker_context,
