...
start_service() {
    ...
    container="${SANITIZER_CONTAINER_NAME:-cppboostexample-inventoryservice-$sanitizer}"
    ...
    docker run -d --name "$container" \
      ...
      "${port_args[@]}" \
      -v "cppboostexample_inventoryservice_cpp-cmake-build:/workspace/build" \
      -w /workspace/source \
      -e SANITIZER_HOLD=1 \
      "inventoryservice-cpp-build" \
      /bin/bash -lc \
